			...
			if (ctx.get().getDirection() == NetworkDirection.PLAY_TO_SERVER) {
				for (PlayerEntity player : PlayerEventsHandler.players) {
					if (player.getUniqueID().toString().contentEquals(pkt.uuid.toString())) {
						int rageAmount = PlayerEventsHandler.rage.get(PlayerEventsHandler.players.indexOf(player));
						if(ActiveAbility.getList().get(pkt.spellID).getCost() <= rageAmount) {
							...
							PlayerEventsHandler.rage.set(PlayerEventsHandler.players.indexOf(player), rageAmount);
