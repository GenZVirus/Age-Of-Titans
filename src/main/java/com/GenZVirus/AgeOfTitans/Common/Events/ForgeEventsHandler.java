...
public class ForgeEventsHandler {
	...
	public static void AOTLavaImmunity(ItemTossEvent event) {
		if (event.getEntityItem().getItem().getItem().equals(ItemInit.FRUIT_OF_THE_GODS.get())) {
			...
			event.getEntityItem().setInvulnerable(true);
			event.getEntityItem().lifespan = 72000;
		} else if (event.getEntityItem().getItem().getItem().equals(ItemInit.HEAVY_TITANIUM_HELMET.get())) {
			event.getEntityItem().setInvulnerable(true);
			event.getEntityItem().lifespan = 72000;
		} else if (event.getEntityItem().getItem().getItem().equals(ItemInit.HEAVY_TITANIUM_CHESTPLATE.get())) {
			event.getEntityItem().setInvulnerable(true);
			event.getEntityItem().lifespan = 72000;
		} else if (event.getEntityItem().getItem().getItem().equals(ItemInit.HEAVY_TITANIUM_LEGGINGS.get())) {
			event.getEntityItem().setInvulnerable(true);
			event.getEntityItem().lifespan = 72000;
		} else if (event.getEntityItem().getItem().getItem().equals(ItemInit.HEAVY_TITANIUM_BOOTS.get())) {
			event.getEntityItem().setInvulnerable(true);
			event.getEntityItem().lifespan = 72000;
	...
	public static void AOTOnApplesEaten(LivingEntityUseItemEvent.Finish event) {
		...
		if (event.getItem().getItem().equals(ItemInit.FRUIT_OF_THE_GODS.get())) {
			...
			for (PlayerEntity playerMSG : PlayerEventsHandler.players) {
